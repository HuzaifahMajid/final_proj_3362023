...
function validateForm() {
    var selectedFlight = document.querySelector('input[name="flight"]:checked');

    if (!selectedFlight) {
        alert('Please select a flight before submitting.');
        return false;
    }
	...
	function sortTable(columnName, sortOrder) {
		...
		while (switching) {
			...
			for (i = 1; i < rows.length - 1; i++) {
				...
				if ((sortOrder === 'asc' && x.localeCompare(y) > 0)
						|| (sortOrder === 'desc' && x.localeCompare(y) < 0)) {
					shouldSwitch = true;
					break;
				}
			}
...
<%
    out.println("console.log('FLIGHTTYPE Attribute: " + String.valueOf(session.getAttribute("flightType")) + "');");
    out.println("console.log('RETURNDATES Attribute: " + String.valueOf(session.getAttribute("returnDates")) + "');");
%>
</script>
</head>
...
<body>
	...
	//	String flightType = request.getParameter("flightType");
		...
		%>
		<script>
		console.log(session.getAttribute("roundtrip"));
		console.log(session.getAttribute("flightType"));
		</script>
	<% 
 ...
 -->
	...
	<form action="reserve_flight.jsp" method="post" onsubmit = "return validateForm();">
		...
		<table border="5" class="sortable">
			<tr>
				...
				<th><select id="takeoffSorting"
					onchange="sortTable('DepartureTime',this.value)">
						<option value="asc">ASC</option>
						<option value="desc">DESC</option>
				</select> <a href="#"
					onclick="sortTable('DepartureTime', document.getElementById('takeoffSorting').value)"></a>
					Take off Time</th>


				<th>Arrival Airport</th>
				<th><select id="landingSorting"
					onchange="sortTable('ArrivalTime', this.value)">
						<option value="asc">ASC</option>
						<option value="desc">DESC</option>
				</select> <a href="#"
					onclick="sortTable('ArrivalTime', document.getElementById('landingSorting').value)"></a>
					Landing Time</th>
				<th>Stops</th>
				<th><select id="durationSort"
					onchange="sortTable('duration', this.value)">
						<option value="asc">ASC</option>
						<option value="desc">DESC</option>
				</select> <a href="#"
					onclick="sortTable('duration', document.getElementById('durationSort').value)"></a>
					Duration</th>

				<th><select id="priceSorting"
					onchange="sortTable('price', this.value)">
						<option value="asc">ASC</option>
						<option value="desc">DESC</option>
				</select> <a href="#"
					onclick="sortTable('price', document.getElementById('priceSorting').value)"></a>
					Price</th>
			...
			<tr>
				<td><input type="radio" required name="flight"
					value="<%=resultSet.getString("flightNumber")%>"
					id="<%=resultSet.getString("flightNumber")%>"></td>
